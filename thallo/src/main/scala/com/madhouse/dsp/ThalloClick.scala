...
/**
  ...
  */
object ThalloClick {
  val log: Logger = LoggerFactory.getLogger(ThalloClick.getClass)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("ThalloClick")
      .set("spark.streaming.kafka.maxRatePerPartition", s"$clkMaxRatePerPartition")
    val ssc = new StreamingContext(sparkConf, Seconds(clkStreamingInterval))

    val topicsSet = clkTopic.split(",").toSet
    var kafkaParams = Map[String, String](
      "metadata.broker.list" -> bootstrapServers,
      "group.id" -> ("ThalloClick_" + clkTopic.replaceAll(",", "_"))
    )

    val fromOffsets = getOffset("clk", offset)
    ...
    logBeforeDecoded.repartition(2).foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        ...
        val jsonDs = rdd.map(r => {
          val a = AvroUtils.decode(r, ClickTrack.SCHEMA$).asInstanceOf[ClickTrack]
        ...
        for (ts <- tss) {
          ...
          val path = hdfsBasePath + "/" + clkTopic + "/" + ts
          log.info(s"##### start to write df of logs to hdfs, path is $path")
          df.coalesce(1).write.format("parquet").mode("append").save(path)
          log.info(s"##### saving df of $clkTopic logs to hdfs finished...")
        }
        jsonDs.unpersist()
        log.info("##### start to write offset to zookeeper...")
        writeOffset("clk", offsetRanges)
        log.info("##### writing offset to zookeeper finished...")
      } else {
        showOffset("clk", offsetRanges)
        log.info(s"##### rdd of $clkTopic logs from kafka is empty, and no needing update offset in zookeeper...")
      }
    })
