...
object ThalloRequest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("ThalloRequest")
      .set("spark.streaming.kafka.maxRatePerPartition", s"$requestMaxRatePerPartition")
    val ssc = new StreamingContext(sparkConf, Seconds(requestStreamingInterval))

    val topicsSet = requestTopic.split(",").toSet
    var kafkaParams = Map[String, String](
      "metadata.broker.list" -> bootstrapServers,
      "group.id" -> ("ThalloRequest_" + requestTopic.replaceAll(",", "_"))
    )

    val fromOffsets = getOffset("request", offset)
    ...
    logBeforeDecoded.repartition(2).foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        ...
        for (ts <- tss) {
          ...
          val path = hdfsBasePath + "/" + requestTopic + "/" + ts
          log.info(s"##### start to write df of logs to hdfs, path is $path")
          df.coalesce(1).write.format("parquet").mode("append").save(path)
          log.info(s"##### saving df of $requestTopic logs to hdfs finished...")
        }
        jsonDs.unpersist()
        log.info("##### start to write offset to zookeeper...")
        writeOffset("request", offsetRanges)
        log.info("##### writing offset to zookeeper finished...")
      } else {
        showOffset("request", offsetRanges)
        log.info(s"##### rdd of $requestTopic logs from kafka is empty, and no needing update offset in zookeeper...")
      }
    })
