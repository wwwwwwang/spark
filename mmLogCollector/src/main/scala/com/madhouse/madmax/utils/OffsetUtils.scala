...
object OffsetUtils extends Serializable {
  ...
  def getOffset(logType: String, offset: String): Map[TopicAndPartition, Long] = {
    ...
    if (children > 0) {
      log(s"####topic($topic) has $children paritions...")
      ...
      if (offset.equalsIgnoreCase(EARLIEST)) {
        log("#### the offset has been set to kafka smallest offset...")
        ...
        log("#### the offset has been set to kafka largest offset...")
        for (i <- 0 until children) {
          val tp = TopicAndPartition(topic, i)
        ...
        log("#### the offset is got from zk, and updated with kafka true offset...")
        for (i <- 0 until children) {
          val tp = TopicAndPartition(topic, i)
      ...
      log(s"####topic($topic) has no children in zookeeper..")
    }
    fromOffsets
  }
  ...
  def writeOffset(logType: String, offsetRanges: Array[OffsetRange]): Unit ={
    ...
    for (o <- offsetRanges) {
      ...
      log(s"#### topic:${o.topic}, partition:${o.partition}, fromoffset:${o.fromOffset}, untiloffset:${o.untilOffset} #######")
    }
  }
